...
Attachment pipeline, share-intent ingestion, on-device OCR, expense capture, duplicate
detection, outbox to XInfo, expense status flow-back, nightly reconciliation.
...
## 4. Open questions to resolve before Phase 1 build

**XInfo contract** (blocking Phase 3, needed early for Phase 1 customer sync):
...
3. Expense intake: exact field list, category codes, receipt mode (URL vs base64), idempotency support?
...
**Operational:**
7. Which OIDC provider exactly — Keycloak fronting AD, or an existing IdP?
8. Are devices enrolled in MDM? Which one? (Changes how much we can enforce.)
9. Is outbound internet allowed for FCM/APNs?
10. Expected fleet size at launch and in 3 years?
11. Any jurisdictions outside India in scope? (Changes the privacy/consent posture.)

**Product:**
12. Should a rep be able to create a customer in the field, or only propose one to XInfo?
13. Is manager pre-approval of tours required, or is planning self-service?
14. Do you need per-diem/mileage computed here, or does XInfo own all money math?
15. Which languages does the app need? (Affects form templates and OCR.)

## 5. What I would build first, if you want a single next step
